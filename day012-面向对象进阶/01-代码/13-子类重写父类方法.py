...
s = Student('tony', 20,'ss')  # 调用了父类的__init__方法
s.sleep()  # 调用了Student的sleep方法(sleep方法被重写了  覆盖)
print(Student.__mro__)  # method resolution order 方法查找解决调用顺序

# 1.子类的实现和父类的实现完全不一样，子类可以选择重写父类的方法
# 2.子类在父类的基础上又有了有更多的实现
